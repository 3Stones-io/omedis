defmodule Omedis.EventTest do
  ...
  describe "by_organisation/1" do
    test "returns events for a specific organisation", %{
      activity: activity,
      organisation: organisation,
      user: user
    } do
      ...
      {:ok, _event_2} =
        create_event(another_organisation, %{
          activity_id: activity.id,
          user_id: user.id
        })

      {:ok, result} =
        Event.by_organisation(%{organisation_id: organisation.id},
          actor: user,
          tenant: organisation
        )
  ...
  describe "by_organisation_today/1" do
    test "returns events created today for the specific organisation", %{
      organisation: organisation,
      user: user,
      activity: activity
    } do
