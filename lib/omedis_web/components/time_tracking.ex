defmodule OmedisWeb.TimeTracking do
  ...
  def dashboard_component(assigns) do
    ...
    <div class="w-[100%] flex flex-col gap-1 ">
      <%= if Enum.empty?(@activities) do %>
        <.no_log_activities language={@language} />
      <% else %>
        <.dashboard_card
  ...
  def no_log_activities(assigns) do
    ~H"""
    <div class="w-[100%] h-[30vh] flex justify-center items-center">
      <%= with_locale(@language, fn -> %>
        <p>
          <%= gettext("No log activities are defined yet.") %>
        </p>
      <% end) %>
    </div>
    """
  ...
  @doc """
  The activities button component. This holds the activities button.
  """
  ...
  def activities_button(assigns) do
    ~H"""
    <div class="flex flex-col gap-0">
      <div class="flex flex-row gap-2 items-center">
        <p
          :if={@active_activity_id == @activities.id}
        ...
        <button
          ...
          phx-value-activity_id={@activities.id}
          style={"background-color: #{@activities.color_code};"}
        >
  ...
  def counter_for_time_taken_by_current_task(assigns) do
    ~H"""
    <%= for log_entry <- @activities.log_entries |> Enum.filter(fn x -> x.created_at |> DateTime.to_date == Date.utc_today  end)   do %>
